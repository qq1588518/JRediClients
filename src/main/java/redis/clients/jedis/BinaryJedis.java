...
public class BinaryJedis implements BasicCommands, BinaryJedisCommands, MultiKeyBinaryCommands,
	...
	public BinaryJedis(final String host, final int port, final boolean ssl, final SSLSocketFactory sslSocketFactory,
	...
	public BinaryJedis(URI uri, final SSLSocketFactory sslSocketFactory, final SSLParameters sslParameters,
			final HostnameVerifier hostnameVerifier) {
	...
	private void initializeClientFromURI(URI uri) {
		...
		if (password != null) {
			...
			if (!"OK".equalsIgnoreCase(reply)) {
				logger.error("Send AUTH Command Error! reply={}" , reply);
			}
		}
		...
		if (dbIndex > 0) {
			...
			String reply = client.getStatusCodeReply();
			if (!"OK".equalsIgnoreCase(reply)) {
				logger.error("Send SELECT Command Error! reply={}" , reply);
			}
		}
	}
	...
	private void initializeClientFromURI(URI uri, final SSLSocketFactory sslSocketFactory,
		...
		client = new Client(uri.getHost(), uri.getPort(), uri.getScheme().equals("rediss"), sslSocketFactory,
	...
	@Deprecated
	/**
	 * This method is deprecated due to its error prone and will be removed on next
	 * major release You can use multi() instead
	 * 
	...
	protected void checkIsInMultiOrPipeline(final String... keys) {
		checkIsInMultiOrPipeline();
		client.watch(keys);
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send WATCH Command Error! reply={}" , reply);
		}
		return;
	}
	
	protected void checkIsInMultiOrPipeline(final byte[]... keys) {
		checkIsInMultiOrPipeline();	
		client.watch(keys);
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
	...
	protected void checkIsInMultiOrPipeline(String key) {
		checkIsInMultiOrPipeline();	
	...
	protected void checkIsInMultiOrPipeline(byte[] key) {
		checkIsInMultiOrPipeline();
	...
	public String unwatch() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send UNWATCH Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String ping() {
		...
		if (!"PONG".equalsIgnoreCase(reply)) {
			logger.error("Send PING Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	@Deprecated
	/**
	 * This method is deprecated due to its error prone with multi and will be
	 * removed on next major release You can use pipelined() instead
	 * 
	 * @see https://github.com/xetorthio/jedis/pull/498
	 */
	...
	public String flushDB() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send FLUSHDB Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String quit() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send QUIT Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String debug(final DebugParams params) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send DEBUG Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String save() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send SAVE Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String bgsave() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send BGSAVE Command Error! reply={}" , reply);
		}
		return reply;
	}
	 ...
	 * <a href="http://code.google.com/p/redis/wiki/AppendOnlyFileHowto">Append Only
	 * File Howto</a>.
	 * <p>
	...
	public String bgrewriteaof() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send BGREWRITEAOF Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String shutdown() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send SHUTDOWN Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public void monitor(final JedisMonitor jedisMonitor) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send MONITOR Command Error! reply={}" , reply);
		}
	...
	public String slaveof(final String host, final int port) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send SLAVEOF Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String set(final byte[] key, final byte[] value) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send SET Command Error! reply={}" , reply);
		}
		return reply;
	}
	 ...
	 *         keys existed 0 if none of the specified keys existed
	 */
	@Override
	...
	public String type(final byte[] key) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send TYPE Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String rename(final byte[] oldkey, final byte[] newkey) {
		checkIsInMultiOrPipeline(oldkey);
		client.rename(oldkey, newkey);
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send RENAME Command Error! reply={}" , reply);
		}
		return reply;
	}
	 ...
	 * 
	 * @see <a href=
	 *      "http://code.google.com/p/redis/wiki/ExpireCommand">ExpireCommand</a>
	 * @param key
	 * @param seconds
	 ...
	 *             Jedis.pexpire(String,long) Set a timeout on the specified key.
	 *             After the timeout the key will be automatically deleted by the
	 *             server. A key with an associated timeout is said to be volatile
	 *             in Redis terminology.
	 *             <p>
	 *             Voltile keys are stored on disk like the other keys, the timeout
	 *             is persistent too like all the other aspects of the dataset.
	 *             Saving a dataset containing expires and stopping the server does
	 *             not stop the flow of time as Redis stores on disk the time when
	 *             the key will no longer be available as Unix time, and not the
	 *             remaining milliseconds.
	 *             <p>
	 *             Since Redis 2.1.3 you can update the value of the timeout of a
	 *             key already having an expire set. It is also possible to undo the
	 *             expire at all turning the key into a normal key using the
	...
	public String flushAll() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send FLUSHALL Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String setex(final byte[] key, final int seconds, final byte[] value) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send SETEX Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String mset(final byte[]... keysvalues) {
		checkIsInMultiOrPipeline(keysvalues);
		client.mset(keysvalues);
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send MSET Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	/**
	 * INCRBY work just like {@link #incr(byte[]) INCR} but instead to increment by
	...
	public String hmset(final byte[] key, final Map<byte[], byte[]> hash) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send HMSET Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String ltrim(final byte[] key, final long start, final long end) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send LTRIM Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String lset(final byte[] key, final long index, final byte[] value) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send LSET Command Error! reply={}" , reply);
		}
		return reply;
	}
	 ...
	 * against the list (a,b,c,hello,x,hello,hello) will have the list
	...
	public byte[] rpoplpush(final byte[] srckey, final byte[] dstkey) {
		checkIsInMultiOrPipeline(srckey);
	...
	public Long sdiffstore(final byte[] dstkey, final byte[]... keys) {
		checkIsInMultiOrPipeline(dstkey);
	...
	public byte[] brpoplpush(byte[] source, byte[] destination, int timeout) {
		checkIsInMultiOrPipeline(source);
	...
	public String configResetStat() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send CONFIG Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	/**
	 * Evaluates scripts using the Lua interpreter built into Redis starting from
	 * version 2.6.0.
	 * <p>
	 * 
	...
	public String scriptFlush() {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send SCRIPT Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String slowlogReset() {
		...
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send SLOWLOG RESET Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String restore(final byte[] key, final int ttl, final byte[] serializedValue) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send RESTORE Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String psetex(final byte[] key, final long milliseconds, final byte[] value) {
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send PSETEX Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String clientKill(final byte[] clientkill) {
		checkIsInMultiOrPipeline();
		client.clientKill(clientkill);
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send CLIENT Kill Command Error! reply={}" , reply);
		}
		return reply;
	}
	...
	public String migrate(final byte[] host, final int port, final byte[] key, final int destinationDb,
		...
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send MIGRATE Command Error! reply={}" , reply);
		}
		return reply;
	}
	 ...
	 * method, we cannot use it, so I had to change the name of the method. Sorry :S
	 */
	@Override
	...
	public String pfmerge(final byte[] destkey, final byte[]... sourcekeys) {
		checkIsInMultiOrPipeline(destkey);
		client.pfmerge(destkey, sourcekeys);
		String reply = client.getStatusCodeReply();
		if (!"OK".equalsIgnoreCase(reply)) {
			logger.error("Send PFMERGE Command Error! reply={}" , reply);
		}
		return reply;
	}
