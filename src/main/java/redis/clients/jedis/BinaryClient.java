...
public class BinaryClient extends Connection {
	...
	public BinaryClient(final String host, final int port, final boolean ssl, final SSLSocketFactory sslSocketFactory,
	...
	public boolean connect() {
		...
		if (!super.isConnected()) {
			ret = super.connect();
			if (ret && (password != null && password != "" && !password.isEmpty())) {
				//auth(password);
			...
			if (ret && (db > 0)) {
				...
				sendCommand(SELECT, toByteArray(Long.valueOf(db).intValue()));
				String reply = getStatusCodeReply();
				if (!"OK".equalsIgnoreCase(reply)) {
