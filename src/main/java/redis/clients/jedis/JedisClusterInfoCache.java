...
public class JedisClusterInfoCache {
	...
	public void renewClusterSlots(Jedis jedis) {
		// If rediscovering is already in process - no need to start one more same
		// rediscovering, just return
		if (!rediscovering) {
			try {
				w.lock();
				rediscovering = true;
	...
	public JedisPool setupNodeIfNotExist(HostAndPort node) {
		...
		try {
			...
			JedisPool nodePool = new JedisPool(poolConfig, node.getHost(), node.getPort(), connectionTimeout, soTimeout,
					password, 0, clientName, false, null, null, null);
	...
	public JedisPool setupNodeIfNotExist(HostAndPort node, boolean ssl) {
		...
		try {
			...
			JedisPool nodePool = new JedisPool(poolConfig, node.getHost(), node.getPort(), connectionTimeout, soTimeout,
					password, 0, null, ssl, null, null, null);
			nodes.put(nodeKey, nodePool);
			return nodePool;
		} finally {
			w.unlock();
